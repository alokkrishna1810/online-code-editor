...
import { Play, Square, Terminal, FileInput, Clock, AlertCircle, CheckCircle, XCircle } from "lucide-react"
...
export function ExecutionPanel({
  ...
  const [executionHistory, setExecutionHistory] = useState<Array<ExecutionResult & { timestamp: Date }>>([])
              ...
              <ScrollArea className="h-[calc(100%-2rem)] border rounded-md">
                <div className="p-3">
                  {isExecuting ? (
                    ...
                    <pre className="text-sm font-mono whitespace-pre-wrap">{result.output || "No output"}</pre>
                  ) : (
                    <div className="text-muted-foreground text-sm">Click "Run Code" to see output here</div>
                  )}
                </div>
              </ScrollArea>
            </div>
          ...
          <TabsContent value="errors" className="mt-4 h-[calc(100%-3rem)]">
            <div className="space-y-3 h-full">
              ...
              <ScrollArea className="h-[calc(100%-2rem)] border rounded-md">
                <div className="p-3">
                  {result?.error ? (
                    <pre className="text-sm font-mono text-red-600 whitespace-pre-wrap">{result.error}</pre>
                  ) : (
                    <div className="text-muted-foreground text-sm">No errors to display</div>
                  )}
                </div>
              </ScrollArea>
            </div>
              ...
              <ScrollArea className="h-[calc(100%-2rem)]">
                <div className="space-y-2">
                  {executionHistory.length > 0 ? (
                      ...
                      <Card key={index} className="p-3">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">{execution.executionTime}ms</span>
                          </div>
                        </div>
                        {execution.output && (
                    ...
                    <div className="text-muted-foreground text-sm text-center py-8">No execution history yet</div>
                  )}
                </div>
              </ScrollArea>
            </div>
