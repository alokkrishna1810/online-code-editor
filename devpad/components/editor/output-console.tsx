...
export function OutputConsole({
  ...
  const exportLogs = () => {
    const logs = filteredMessages
      .map((msg) => {
        const timestamp = showTimestamps ? `[${msg.timestamp.toLocaleTimeString()}] ` : ""
  ...
  const messageTypeCounts = allMessages.reduce(
    (acc, msg) => {
        ...
        <Tabs value={filter} onValueChange={setFilter} className="w-full">
          <TabsList className="grid w-full grid-cols-6 h-8">
            ...
            <TabsTrigger value="error" className="text-xs">
              Error {messageTypeCounts.error > 0 && `(${messageTypeCounts.error})`}
            </TabsTrigger>
            <TabsTrigger value="output" className="text-xs">
              Output {messageTypeCounts.output > 0 && `(${messageTypeCounts.output})`}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
              ...
              filteredMessages.map((message) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <pre
                      className={`text-sm font-mono whitespace-pre-wrap break-words ${getMessageColor(message.type)}`}
                    >
                      {message.content}
                    </pre>
                  </div>
...
export function useConsole() {
  ...
  const addMessage = useCallback((type: ConsoleMessage["type"], content: string, source?: string) => {
    const newMessage: ConsoleMessage = {
      id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
      type,
      content,
  ...
  const log = useCallback((content: string, source?: string) => addMessage("log", content, source), [addMessage])
  const info = useCallback((content: string, source?: string) => addMessage("info", content, source), [addMessage])
  const warn = useCallback((content: string, source?: string) => addMessage("warn", content, source), [addMessage])
  const error = useCallback((content: string, source?: string) => addMessage("error", content, source), [addMessage])
  const success = useCallback(
    (content: string, source?: string) => addMessage("success", content, source),
    [addMessage],
  )
